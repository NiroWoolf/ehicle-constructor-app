...
if len(st.session_state["points"]) >= 2:
    ...
    st.sidebar.warning("Отметьте как минимум 2 точки для калибровки.")

# Отображаем рассчитанный масштаб
if st.session_state["pixels_per_meter"]:
    st.sidebar.success(f"Масштаб: {st.session_state['pixels_per_meter']:.2f} пикс/метр")
else:
    st.sidebar.info("Масштаб не рассчитан.")
